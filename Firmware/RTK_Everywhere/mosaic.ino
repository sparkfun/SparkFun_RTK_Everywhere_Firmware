...
// Check if updateMosaicX5Firmware.txt exists
bool checkUpdateMosaicX5Firmware()
{
    if (online.fs == false)
        return false;

    if (LittleFS.exists("/updateMosaicX5Firmware.txt"))
    {
        if (settings.debugGnss)
            systemPrintln("LittleFS updateMosaicX5Firmware.txt exists");

        // We do not remove the file here. See removeupdateMosaicX5Firmware().
...
void removeUpdateMosaicX5Firmware()
{
    if (online.fs == false)
        return;
    ...
    {
        if (settings.debugGnss)
            systemPrintln("Removing updateMosaicX5Firmware.txt ");

        LittleFS.remove("/updateMosaicX5Firmware.txt");
    }
}

// Force UART connection to MOSAICX5 for firmware update on the next boot by creating updateMosaicX5Firmware.txt in
// LittleFS
bool createMosaicX5Passthrough()
{
    if (online.fs == false)
        return false;
    ...
    {
        if (settings.debugGnss)
            systemPrintln("LittleFS updateMosaicX5Firmware.txt already exists");
        return true;
    }

    File updateMosaicX5Firmware = LittleFS.open("/updateMosaicX5Firmware.txt", FILE_WRITE);
    updateMosaicX5Firmware.close();

    if (LittleFS.exists("/updateMosaicX5Firmware.txt"))
        return true;

    if (settings.debugGnss)
        systemPrintln("Unable to create updateMosaicX5Firmware.txt on LittleFS");
    return false;
}

void beginMosaicX5FirmwareUpdate()
{
    // Note: We cannot increase the bootloading speed beyond 115200 because
    //  we would need to alter the MOSAICX5 baud, then save to NVM, then allow the MOSAICX5 to reset.
    ...
    //  then reconfigures the MOSAICX5 to 115200bps, then resets, but autobaud detection in the MOSAICX5 library is
    //  not yet supported.
    ...
    systemPrintln();
    systemPrintln("Entering MOSAICX5 direct connect for firmware update and configuration. Disconnect this terminal "
                  "connection. Use "
                  "UPrecise to update the firmware. Baudrate: 115200bps. Press the power button to return "
                  "to normal operation.");
    systemFlush();

    // Make sure ESP-UART1 is connected to MOSAICX5
    muxSelectMosaicX5();
    ...
    // Wait for UPrecise to send bootloader trigger (character T followed by character @) before resetting MOSAICX5
    bool inBootMode = false;

    // Echo everything to/from MOSAICX5
    while (1)
    {
        // Data coming from MOSAICX5 to external USB
        if (serialGNSS->available())
            Serial.write(serialGNSS->read());

        // Data coming from external USB to MOSAICX5
        if (Serial.available())
        {
        ...
        if (digitalRead(pin_powerButton) == HIGH)
        {
            ...
            systemPrintln("Exiting MOSAICX5 passthrough mode");
