...
class GNSS_None : public GNSS
{
  protected:
    ...
    bool configureGNSS() {return false;}

    // Set the minimum satellite signal level for navigation.
    bool setMinCnoRadio (uint8_t cnoValue) {return false;}
  ...
  public:
    ...
    bool beginExternalEvent() {return false;}

    // Setup the timepulse output on the PPS pin for external triggering
    // Outputs
    //   Returns true if the pin was successfully setup and false upon
    //   failure
    bool beginPPS() {return false;}

    bool checkNMEARates() {return false;}

    bool checkPPPRates() {return false;}

    // Configure the Base
    // Outputs:
    //   Returns true if successfully configured and false upon failure
    bool configureBase() {return false;}

    // Configure specific aspects of the receiver for NTP mode
    bool configureNtpMode() {return false;}

    // Configure the Rover
    // Outputs:
    //   Returns true if successfully configured and false upon failure
    bool configureRover() {return false;}
    ...
    bool enableRTCMTest() {return false;}
    ...
    uint16_t fileBufferExtractData(uint8_t *fileBuffer, int fileBytesToRead) {return 0;}

    // Start the base using fixed coordinates
    // Outputs:
    //   Returns true if successfully started and false upon failure
    bool fixedBaseStart() {return false;}

    // Return the number of active/enabled messages
    uint8_t getActiveMessageCount() {return 0;}
    ...
    uint8_t getCarrierSolution() {return 0;}

    uint32_t getDataBaudRate() {return 0;}
    ...
    uint16_t getFixAgeMilliseconds() {return 0;}
    ...
    const char * getId() {return "No compiled";}
    ...
    uint8_t getMillisecond() {return _millisecond;}
    ...
    uint32_t getNanosecond() {return 0;}

    uint32_t getRadioBaudRate() {return 0;}

    // Returns the seconds between solutions
    double getRateS() {return 1.0;}

    const char * getRtcmDefaultString() {return "Not compiled";}

    const char * getRtcmLowDataRateString() {return "Not compiled";}

    // Returns the number of satellites in view or zero if offline
    uint8_t getSatellitesInView() {return _satellitesInView;}
    ...
    float getSurveyInMeanAccuracy() {return _horizontalAccuracy;}

    // Return the number of seconds the survey-in process has been running
    int getSurveyInObservationTime() {return 0;}

    float getSurveyInStartingAccuracy() {return 0;}

    // Returns timing accuracy or zero if not online
    uint32_t getTimeAccuracy() {return 0;}
    ...
    bool isConfirmedDate() {return false;}
    ...
    bool isFullyResolved() {return false;}

    bool isPppConverged() {return false;}

    bool isPppConverging() {return false;}
    ...
    int pushRawData(uint8_t *dataToSend, int dataLength) {return dataLength;}

    uint16_t rtcmBufferAvailable() {return 0;}
    ...
    uint16_t rtcmRead(uint8_t *rtcmBuffer, int rtcmBytesToRead) {return 0;}
    ...
    bool setBaudrate(uint32_t baudRate) {return true;}
    ...
    bool setElevation(uint8_t elevationDegrees) {return true;}

    // Enable all the valid messages for this platform
    bool setMessages(int maxRetries) {return true;}

    // Enable all the valid messages for this platform over the USB port
    bool setMessagesUsb(int maxRetries) {return true;}

    // Set the dynamic model to use for RTK
    // Inputs:
    //   modelNumber: Number of the model to use, provided by radio library
    bool setModel(uint8_t modelNumber) {return true;}

    bool setRadioBaudRate(uint32_t baud) {return true;}
    ...
    //   failure
    bool setRate(double secondsBetweenSolutions) {return true;}

    bool setTalkerGNGGA() {return true;}
